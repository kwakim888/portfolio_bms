				...
				<tr>
				  <td>
				    <select name="search_condition" id="search_condition" >
						<option <c:if test="${search_condition eq 'all'}"> selected</c:if> value="all">전체</option>
						<option <c:if test="${search_condition eq 'goodsTitle'}"> selected</c:if> value="goodsTitle">주문내역</option>
						<option <c:if test="${search_condition eq 'orderId'}"> selected</c:if> value="orderId">주문번호</option>
					</select>
					<input type="text" id="searchKeyWord" name="searchKeyWord" value="${searchKeyWord }" size="30" />  
					<input type="button"  value="Search" id="getSearchKeyWord"/>
				  </td>
				</tr>
				<tr>
				  <td>
     ...
     <c:forEach var="item" items="${myOrderHistList }" varStatus="i">
        <c:choose>
          <c:when test="${item.orderId != pre_orderId }">   
            <tr>       
				<td>
				  <a href="${contextPath}/mypage/myOrderDetail.do?orderId=${item.orderId }&goodsId=${item.goodsId }"><strong>${item.orderId }</strong>  </a>
				</td>
				<td >
